...
class Inicio1 extends CI_Controller
{
    ...
    public function index()
    {
        ...
        unset($keys);
        unset($values);
        $data["sel_empleados"] = form_dropdown('id_empleado', $data_usuarios, '1');


        $this->load->view('login/auth-login1', $data);
    }
    ...
    public function layoutBlank()
    {
        ...
        $sql = "SELECT * FROM empleados where id = " . $_POST['id_empleado'];
        $query = $this->db->query($sql);
        $datos = $query->result_array();
        ...
        foreach ($datos as $value) {
            $tx_empleado  = $tx_empleado .$value['descripcion'];
        }

        $data['nb_empleado'] = $tx_empleado;

        $_SESSION["nb_empleado"] = $tx_empleado;
